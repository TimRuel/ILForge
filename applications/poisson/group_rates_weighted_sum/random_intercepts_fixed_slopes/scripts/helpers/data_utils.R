# applications/poisson/group_rates_weighted_sum/random_intercepts_fixed_slopes/scripts/helpers/data_utils.R
...
# Build design matrix compatible with stacked Beta (alpha, gamma, zeta_g)
...
generate_data <- function(config, Beta_0, sigma_alpha) {
  ...
  eta <- X %*% Beta_0 + rep(alpha_g, times = n_per_group)
  theta <- exp(eta)
  Y <- rpois(total_n, t * theta)
