# applications/negative_binomial/process_rates_weighted_sum/naive_rates/scripts/generate_data.R
...
# ✅ Anchor project root
# -------------------------------
suppressMessages(i_am("applications/negative_binomial/process_rates_weighted_sum/naive_rates/scripts/generate_data.R"))
...
phi_0_path         <- here(true_params_dir, "phi_0.rds")
weights_path       <- here(true_params_dir, "weights.rds")
n_per_process_path <- here(true_params_dir, "n_per_process.rds")

if (!file_exists(theta_0_path))       stop("[ERROR] theta_0.rds not found at: ", theta_0_path)
if (!file_exists(phi_0_path))         stop("[ERROR] phi_0.rds not found at: ", phi_0_path)
if (!file_exists(weights_path))       stop("[ERROR] weights.rds not found at: ", weights_path)
if (!file_exists(n_per_process_path)) stop("[ERROR] n_per_process.rds not found at: ", n_per_process_path)

message("[INFO] Loading theta_0 from: ", theta_0_path)
message("[INFO] Loading phi_0 from: ", phi_0_path)
message("[INFO] Loading weights from: ", weights_path)
message("[INFO] Loading n_per_process from: ", n_per_process_path)

theta_0       <- readRDS(theta_0_path)
phi_0         <- readRDS(phi_0_path)
weights       <- readRDS(weights_path)
n_per_process <- readRDS(n_per_process_path)
